use cursive::theme::{
	Color, ColorStyle, ColorType, PaletteColor,
};
use cursive::traits::*;
	...
	match &*background_ascii_art_path {
		Some(ref path) => match fs::read_to_string(path) {
			Ok(ascii_art) => {
				...
				stack.add_fullscreen_layer(Layer::new(
					ascii_view.full_screen(),
				));
			}
